    ...
    pub(super) fn prefill_attention_paged_attn(
        ...
        if hd > 256 && self.prefill_attn_512_k.0 != 0 && seq_len_start == 0 {
            ...
            match (self.kv_dtype, use_br64) {
                ...
                // FibQuant4x: identical launcher (codebook + stride args) — the
                // handle carries the `_4x` module (compiled with `-DFIB_K=2`),
                // and `fibq_codebook_dev` holds the k=2 codebook built in init.
                (KvCacheDtype::FibQuant4x, _) => ops::prefill_attention_paged_fibquant(
                    ctx.gpu,
                    self.prefill_attn_paged_fibquant_4x_k,
                    q_contiguous,
                    kv_cache.k_pool_ptr(self.attn_layer_idx),
                    kv_cache.v_pool_ptr(self.attn_layer_idx),
                    attn_out,
