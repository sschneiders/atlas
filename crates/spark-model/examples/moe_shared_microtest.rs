...
fn f32_to_e4m3(v: f32) -> u8 {
    let mut best = 0u8; let mut be = f32::INFINITY;
    for b in 0..=255u8 { let d = e4m3_to_f32(b); if d.is_finite() { let e = (d - v).abs(); if e < be { be = e; best = b; } } }
    best
}
fn up_bytes(gpu: &dyn GpuBackend, b: &[u8]) -> Result<DevicePtr> { let p = gpu.alloc(b.len().max(16))?; gpu.copy_h2d(b, p)?; Ok(p) }
...
fn main() -> Result<()> {
    ...
    let av: Vec<u16> = (0..k).map(|_| f32_to_bf16(rng.uniform(-1.0, 1.0))).collect();
    let gw: Vec<u8> = (0..n * k).map(|_| f32_to_e4m3(rng.uniform(-0.4, 0.4))).collect();
    let uw: Vec<u8> = (0..n * k).map(|_| f32_to_e4m3(rng.uniform(-0.4, 0.4))).collect();
    ...
    let handle = gpu.kernel("moe_shared_expert_fused_fp8", "moe_expert_gate_up_shared_fp8")?;
    KernelLaunch::new(gpu, handle)
        .grid([div_ceil(n as u32, 8), 1, 2])
        .block([128, 1, 1])
        .arg_ptr(a_p)
        .arg_ptr(dummy).arg_ptr(dummy).arg_ptr(dummy) // gate routed: w_ptrs, scale_ptrs, out
        .arg_ptr(dummy).arg_ptr(dummy).arg_ptr(dummy) // up routed
        .arg_ptr(dummy)                                // expert_indices
        .arg_ptr(gw_p).arg_ptr(gs_p).arg_ptr(go_p)     // shared gate: w, scale, out
        .arg_ptr(uw_p).arg_ptr(us_p).arg_ptr(uo_p)     // shared up
        .arg_u32(n as u32).arg_u32(k as u32).arg_u32(0) // N, K, top_k=0
        .launch(stream)?;
    ...
    let cosv = |g: &[u16], c: &[u16]| -> f64 {
        ...
        for i in 0..n { let x = bf16_to_f32(g[i]) as f64; let y = bf16_to_f32(c[i]) as f64; d += x*y; na += x*x; nb_ += y*y; }
        if na == 0.0 || nb_ == 0.0 { return f64::NAN; }
        d / (na.sqrt() * nb_.sqrt())
    };
    let cg = cosv(&go, &go_c);
    let cu = cosv(&uo, &uo_c);
    println!("gate_cos={cg:.6}  up_cos={cu:.6}");
    for p in [a_p, gw_p, uw_p, gs_p, us_p, go_p, uo_p, dummy] { gpu.free(p).ok(); }
    if cg >= COSINE_GATE && cu >= COSINE_GATE {
        println!("RESULT: PASS"); Ok(())
    } else {
        println!("RESULT: FAIL (shared-expert gate/up GEMV diverges)"); std::process::exit(1);
    }
}
