...
fn main() {
    ...
    // NVIDIA + SCALE expose the CUDA module API → real `all_ptx_sets()`
    // registry; Metal gets a stub (it has its own registry path).
    let cuda_api = compute_target.uses_cuda_module_api();
