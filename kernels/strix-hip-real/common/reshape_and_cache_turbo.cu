...
__device__ __forceinline__ __nv_fp8_storage_t float_to_fp8(float val) {
    ...
    // SCALE/gfx1151 and HIP/gfx1151: the `cvt.rn.satfinite.e4m3x2.f32` inline
    // PTX has no codegen on AMD. scl_enc_fp8 is a software E4M3 encoder with
    // identical SATFINITE+E4M3 semantics — numerically exact, not an
    // approximation. SCALE defines __SCALE__; a native hipcc build defines
    // __HIP_PLATFORM_AMD__ instead, so both must take the software path.
