...
extern "C" __global__ void w4a16_gemv(
    ...
    // Process 16 K-values per iteration (2× uint4 activation + uint64 weight).
    // The single-token w4a16_gemv stays at uint64 (16 FP4) weight loads: a
    // wider uint4 (32 FP4) variant was measured ~10% SLOWER here on gfx1151
    // (extra VGPR pressure for the 16-elem activation array drops occupancy).
    // The batch2/dual_batch2 variants below DO use uint4 because amortizing the
    // weight load across 2 token vectors changes the register/BW trade-off.
...
extern "C" __global__ void w4a16_gemv_batch2(
    ...
    const unsigned int half_K = K / 2;
    const unsigned int num_groups = K / GROUP_SIZE;
    // 32 K-values per iteration: one uint4 weight load (32 FP4), shared across
    // both token activation vectors. Wider weight loads improve memory-level
    // parallelism. 32 spans 2 GROUP_SIZE=16 scale groups. K mult of 32 here.
    ...
    for (unsigned int k32 = lane; k32 < K32; k32 += threads_per_out) {
        ...
        uint4 a0v0 = ((const uint4*)A)[k32 * 4 + 0];
        uint4 a0v1 = ((const uint4*)A)[k32 * 4 + 1];
        uint4 a0v2 = ((const uint4*)A)[k32 * 4 + 2];
        uint4 a0v3 = ((const uint4*)A)[k32 * 4 + 3];
        uint4 a1v0 = ((const uint4*)A1)[k32 * 4 + 0];
        uint4 a1v1 = ((const uint4*)A1)[k32 * 4 + 1];
        uint4 a1v2 = ((const uint4*)A1)[k32 * 4 + 2];
        uint4 a1v3 = ((const uint4*)A1)[k32 * 4 + 3];
        const unsigned int a0_raw[16] = {
            a0v0.x, a0v0.y, a0v0.z, a0v0.w, a0v1.x, a0v1.y, a0v1.z, a0v1.w,
            a0v2.x, a0v2.y, a0v2.z, a0v2.w, a0v3.x, a0v3.y, a0v3.z, a0v3.w};
        const unsigned int a1_raw[16] = {
            a1v0.x, a1v0.y, a1v0.z, a1v0.w, a1v1.x, a1v1.y, a1v1.z, a1v1.w,
            a1v2.x, a1v2.y, a1v2.z, a1v2.w, a1v3.x, a1v3.y, a1v3.z, a1v3.w};

        uint4 w_packed = *(const uint4*)(B_packed + (unsigned long long)n * half_K + (unsigned long long)k32 * 16);
        ...
        for (int q = 0; q < 4; q++) {
            ...
            for (int b = 0; b < 4; b++) {
                ...
                unsigned int a0r = a0_raw[q * 4 + b];
                __nv_bfloat16 a0_lo, a0_hi;
                *(unsigned short*)&a0_lo = (unsigned short)(a0r & 0xFFFF);
                *(unsigned short*)&a0_hi = (unsigned short)(a0r >> 16);
                acc0 += __bfloat162float(a0_lo) * w_lo;
                acc0 += __bfloat162float(a0_hi) * w_hi;

                unsigned int a1r = a1_raw[q * 4 + b];
                __nv_bfloat16 a1_lo, a1_hi;
                *(unsigned short*)&a1_lo = (unsigned short)(a1r & 0xFFFF);
                *(unsigned short*)&a1_hi = (unsigned short)(a1r >> 16);
    ...
    #pragma unroll
    for (int offset = WARP_SIZE / 2; offset > 0; offset >>= 1) {
        acc0 += __shfl_down_sync(0xFFFFFFFFULL, acc0, offset);
...
extern "C" __global__ void w4a16_gemv_dual_batch2(
    ...
    // 32 K-values per iteration (uint4 weight load, shared across both tokens).
    const unsigned int K32 = K_in / 32;
    ...
    for (unsigned int k32 = lane; k32 < K32; k32 += threads_per_out) {
        ...
        unsigned int sg0 = base_k / GROUP_SIZE;
        float scale_a = scl_fp8(B_scale[(unsigned long long)n * num_groups + sg0]) * s2;
        float scale_b = scl_fp8(B_scale[(unsigned long long)n * num_groups + sg0 + 1]) * s2;
