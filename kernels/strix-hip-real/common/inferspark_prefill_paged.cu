...
// Paged Prefill Flash Attention — BF16 KV cache variant. HIP/gfx1151 port.
//
// Reads contiguous BF16 Q, K/V from BF16 paged cache via block_table.
// cp.async PTX from the NVIDIA source is replaced by synchronous 16-byte
// (uint4) smem copies; the shared compute header brackets each tile load
// with __syncthreads(). See prefill_paged_compute.cuh for the WMMA port.

#include <cuda_bf16.h>

// BF16 tile loader: synchronous uint4 copy from paged cache to shared memory.
    ...
    do { \
        ...
        for (unsigned int _i = (t); _i < TILE_CHUNKS; _i += (stride)) { \
            ...
            if (_pos < (kv_l)) { \
                ...
                *((uint4*)&(smem)[_row][_col]) = *((const uint4*)_gm); \
