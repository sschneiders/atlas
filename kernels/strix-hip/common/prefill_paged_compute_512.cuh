...
extern "C" __global__ void KERNEL_NAME(
    ...
    for (unsigned int kv_block = 0; kv_block < num_kv_blocks; kv_block++) {
        ...
        if (warp_id < 2) {
            ...
            sum0+=__shfl_xor_sync(0xFFFFFFFFULL,sum0,1); sum0+=__shfl_xor_sync(0xFFFFFFFFULL,sum0,2);
            sum1+=__shfl_xor_sync(0xFFFFFFFFULL,sum1,1); sum1+=__shfl_xor_sync(0xFFFFFFFFULL,sum1,2);
            l_r0+=sum0; l_r1+=sum1;
            ...
            /*PTX-neutralized*/  // empty: balance with V load
        } else {
